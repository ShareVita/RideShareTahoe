...
export async function POST(request: NextRequest) {
  try {
    ...
    const ridesToInsert = ridesData.map((ride) => {
      const postingType = ride.posting_type || 'driver';
      const totalSeats = ride.total_seats || 1;

      // Validate available_seats does not exceed total_seats
      if (typeof ride.available_seats === 'number' && ride.available_seats > totalSeats) {
        throw new z.ZodError([
          {
            code: z.ZodIssueCode.custom,
            message: 'available_seats cannot exceed total_seats',
            path: ['available_seats'],
          },
        ]);
      }

      // Derive available_seats server-side: drivers get total_seats, passengers get null
      const availableSeats = postingType === 'driver' ? totalSeats : null;
