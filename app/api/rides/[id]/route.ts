  ...
  try {
    ...
    if (seriesGroupId && ride.is_recurring) {
      if (applyTo === 'series') {
        ...
        // Use contains filter for recurring_days (array) when round trip, eq for round_trip_group_id otherwise
        let query = supabase.from('rides').update(updateData);
        if (ride.is_round_trip) {
          query = query.contains('recurring_days', [seriesGroupId]);
        } else {
          query = query.eq('round_trip_group_id', seriesGroupId);
        }
        const { data: updatedRides, error: updateError } = await query
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (seriesGroupId && ride.is_recurring) {
      if (applyTo === 'series') {
        // Delete ALL rides in the series (including past ones)
        let query = supabase.from('rides').delete();
        if (ride.is_round_trip) {
          query = query.contains('recurring_days', [seriesGroupId]);
        } else {
          query = query.eq('round_trip_group_id', seriesGroupId);
        }
        const { data: deletedRides, error: deleteError } = await query
          .eq('poster_id', user.id)
          .select('id');
