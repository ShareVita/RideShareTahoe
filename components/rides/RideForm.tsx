  ...
  const buildRidesFromDates = (
    ...
    for (const date of dates) {
      ...
      if (entry.isRoundTrip) {
        // Round trips get their own group ID to link departure/return legs
        const roundTripGroupId = crypto.randomUUID();
        // For multi-date round trips, store series ID in recurring_days to enable series management
        const recurringDays = isMultiDate && seriesGroupId ? [seriesGroupId] : null;
